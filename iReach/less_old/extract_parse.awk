...
FNR >= data_start_row && NF == num_fields {
    if(count < FNR)
    {
        ...
        gap = FNR - count
        if(gap < period)
            print "WARNING: Data Gap: surface" surface "-sub" subject "-mvt" movement ": Lines(" count "-" FNR-1 ") Size(" gap ")" >> logfile 
        else
            print "ERROR: Large Data Gap: surface" surface "-sub" subject "-mvt" movement ": Lines(" count "-" FNR-1 ") *** Size(" gap ") ***" >> logfile 
        count = FNR
    }
    count++
        ...
        for(field in valid_fields[node])
        {
            ...
            if(bufferFull)
            {
                ...
                if(record + (period - overlap) > last_row)
                {
                    ...
                    {
                        i = (i % period)
                        if(i == ((buf_index + period) - overlap + 1) % period)
                            break
                        else
                            delete buf[field][i++]
                    }
