...
#EXTRACT_OUT_FILE=./data/features-window${WINDOW}-overlap${OVERLAP}.csv

TMP_DIR=tmp
IN_DIR=/media/jsj/DROPSHIP/data/raw
PARSE_OUT_DIR=/media/jsj/DROPSHIP/data/parsed_raw
EXTRACT_OUT_DIR=/media/jsj/DROPSHIP/data
...
while [ -d "${IN_DIR}/surfaceV/S${sub}" ] ; do
    ...
    for (( i=0; i<${lengthV}; i++ )) ; do
        ...
        gawk -f "$AWK_SCRIPT" \
            -v surface="V" -v subject=$sub -v movement=${i} \
        ...
        if [ "$?" -gt 0 ]; then
            rm --verbose "${PARSE_OUT_DIR}/sub${sub}-mvt${i}"*
            rm --verbose "${EXTRACT_OUT_DIR}/sub${sub}-mvt${i}"*
            continue
        fi

        # Process and extract raw data from surfaceH
        gawk -f "$AWK_SCRIPT" \
            -v surface="H" -v subject=$sub -v movement=${i} \
            -v window=$WINDOW -v overlap=$OVERLAP -v data_gap_limit=$DATA_GAP_LIMIT \
            -v extract_outdir="${EXTRACT_OUT_DIR}" -v parse_outdir="${PARSE_OUT_DIR}" \
            -v num_lines=$(wc -l < "${fileArrayH[$i]}") -v doParse=$PARSE_RAW \
            "${fileArrayH[$i]}"

        cat ${TMP_DIR}/mote*.csv >> ${EXTRACT_OUT_DIR}/features.csv
